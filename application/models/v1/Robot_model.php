...
class Robot_model extends CI_Model{
    ...
    public function read_site($user_id, $input_parameters){
        ...
        if ($using_cache) {
            ...
            // if refresh request, then do the same thing as a normal request
            // but don't do a early return of expired snapshots
            // coalesce the refresh requests into one refesh request

            // the only requests that are not coalesced are cache=false requests
            // this is because we rely on the cache to coalesce tasks

            $no_early_stale_return = $refresh;
    ...
    protected function handle_cache_stampede (
        ...
        if ($this->acquire_lock($lock, LOCK_EX, 0)) {
            ...
            // if this request was a refresh request, we don't bother doing an early stale cache return
            // this is because multiple refresh requests should coalesce into 1 refresh request
            // but semantically they all deserve a refreshed snapshot
            // so early returning a stale snapshot would violate the semantics
            if (!$no_early_stale_return) {
