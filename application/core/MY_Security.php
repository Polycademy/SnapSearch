...
class MY_Security extends CI_Security{
	...
	//note that does not work for API based applications, that is applications that are intended to be called programmatically over HTTP
	//You would need to cancel CSRF verification globally, or use the CSRF ignore on the API routes, OR just don't use cookies for authentication. Use localstorage and access token. The session id can be managed via the localstorage too! To use localstorage, use access token provided via OAuth, Hawk doesn't really work due to the need for a shared secret that is permanent.
